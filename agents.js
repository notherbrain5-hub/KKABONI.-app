...
// Agent 1: 데이터 수집 자동화 아키텍트 (Crawling & Parsing Pipeline)
window.Agent1 = {
  // 1. 외부 데이터 수집 (Crawling)
  fetchVendorData: async (url) => {
    try {
      // 브라우저 환경에서의 fetch 시뮬레이션 (실제 백엔드/퍼페티어 등 환경에 맞춰 확장 가능)
      ...
      // 4. 에러 핸들링 (Anti-Block)
      console.error("[Crawling Failed] 타겟 사이트 구조 변경됨");
      return null;
    }
  },

  // 2. AI 파싱 및 추가금 산출 (Parsing)
  calculateHiddenCosts: (rawData) => {
    if (!rawData) return { expectedHiddenCost: 0, foundKeywords: [] };
    
    let expectedHiddenCost = 0;
    let foundKeywords = [];
    
    // 숨겨진 추가금 키워드 정규식 및 예상 비용 매핑 딕셔너리
    const keywordMap = [
  ...
  // 3. Supabase(DB) 자동 적재 로직
  syncToDatabase: async (vendorData) => {
    try {
      // Supabase REST API 연동을 가정한 비동기 처리 시뮬레이션
      /* 
      const supabaseUrl = 'https://<project-ref>.supabase.co/rest/v1/Vendors';
      const response = await fetch(supabaseUrl, {
        ...
        headers: {
          ...
          'Prefer': 'resolution=merge-duplicates' // Upsert 로직
        },
      ...
      */
      ...
      console.log("[DB Sync] Supabase Vendors 테이블에 데이터 적재 완료:", vendorData);
      return true;
    } catch (error) {
      console.error("[DB Sync Failed] 데이터베이스 적재 실패", error);
      return false;
    }
  },

  // 전체 파이프라인 실행 유틸리티
  runPipeline: async (vendorName, url, basePrice) => {
    const rawData = await window.Agent1.fetchVendorData(url);
    if (!rawData) return null; // 에러 핸들링에 의해 다음 작업으로 패스

    const { expectedHiddenCost, foundKeywords } = window.Agent1.calculateHiddenCosts(rawData);
    const totalCost = basePrice + expectedHiddenCost;

    const vendorData = {
      업체명: vendorName,
      기본가: basePrice,
      추가금: expectedHiddenCost,
      총비용: totalCost,
      포함내역: ["기본 패키지"],
      불포함내역: foundKeywords
    };

    await window.Agent1.syncToDatabase(vendorData);
    return vendorData;
  }
};
