// src/config/mockAgents.js
// 기존 agents.js에 있던 더미 데이터 및 Agent2, Agent3, Agent4 시뮬레이션 로직
    ...
    for (let i = 0; i < 20; i++) {
      ...
      if (cat.id === 'sdm') {
        ...
        hiddenCosts = [
          ...
          { item_name: "가드링 추가", average_cost: 300000, is_mandatory: false, occurrence: 85 }
        ];
        features = ["자체 공방", "평생 A/S"];
        notes = "상담 시 다이아 등급별 추가금을 명확히 확인하세요.";
      } else if (cat.id === 'suit') {
        vendorName = `킹스맨 테일러 ${i + 1}`;
        basePrice = Math.floor(Math.random() * 800000) + 700000;
        hiddenCosts = [
          { item_name: "수입 원단 업그레이드", average_cost: 400000, is_mandatory: false, occurrence: 88 },
          { item_name: "수제화 추가", average_cost: 250000, is_mandatory: false, occurrence: 60 }
        ];
        features = ["영국 원단", "비스포크"];
        notes = "국내 원단과 수입 원단 간의 가격 차이가 큽니다.";
      } else if (cat.id === 'hanbok') {
        vendorName = `비단길 한복 ${i + 1}`;
        basePrice = Math.floor(Math.random() * 300000) + 200000;
        hiddenCosts = [
          { item_name: "실크 원단 변경", average_cost: 150000, is_mandatory: false, occurrence: 55 },
          { item_name: "속치마/장신구 대여", average_cost: 50000, is_mandatory: true, occurrence: 90 }
        ];
        features = ["파스텔톤", "맞춤 대여"];
        notes = "소품 대여비가 별도인지 확인이 필요합니다.";
      } else {
...
export const VENDOR_DB = generateDummyData();
